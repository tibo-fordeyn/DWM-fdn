...
static float
getbrightness(const char *hexcol)
{
    if (hexcol[0] == '#')
        hexcol++;

    unsigned int r, g, b;
    if (sscanf(hexcol, "%02x%02x%02x", &r, &g, &b) != 3) {
        /* als parsing mislukt, keer 0 terug (heel donker) */
        return 0.0f;
    }

    /* formule voor relatieve luminantie (sRGB): 
       Y = 0.2126*R + 0.7152*G + 0.0722*B, met R,G,B ∈ [0..1] */
    float rf = r / 255.0f;
    float gf = g / 255.0f;
    float bf = b / 255.0f;
...
loadxrdb()
{
	...
	if (display != NULL) {
		...
		if (resm != NULL) {
		    ...
		    /* Zet foreground automatisch zwart of wit, afhankelijk van hoe licht normbgcolor is */
    	float brightness = getbrightness(normbgcolor);
    	if (brightness > 0.5f) {
        	/* Bar is licht, dus zwarte tekst */
        	strcpy(normfgcolor, "#000000");
    	} else {
        	/* Bar is donker, dus witte tekst */
        	strcpy(normfgcolor, "#ffffff");
    	}
...
//	c->y = c->mon->my + (c->mon->mh - HEIGHT(c)) / 2;
	c->x = c->mon->wx + (c->mon->ww - WIDTH(c)) / 2; // bar probleem
