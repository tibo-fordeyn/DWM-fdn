...
![Alt-tekst](/home/dyntif/persoonlijk/dwm-fdn/screenshots/blauw.png)
![Alt-tekst](/home/dyntif/persoonlijk/dwm-fdn/screenshots/geel.png)
![Alt-tekst](/home/dyntif/persoonlijk/dwm-fdn/screenshots/grijs.png)
...
```bash
$ sudo pacman -S base-devel git networkmanager networkmanager-runit firefox qutebrowser picom newsboat htop vim mpv yt-dlp zathura python unclutter dash
```
```bash
...
$ cd yay
$ makepkg -si
$ run sudo make install on alle suckless directories
```
it seems to me like the dwm and wal installation is a chicken and egg thing.
You can't make them without wal, but you can't seem to get wal to generate the files without them, I think.
There likely is a workaround, but I'd just leave the #include out on the first install, and that'll work too.

``` bash
$ yay -S wal 
$ execute a wal command on a theme so that the needed color files are generated
``` 
``` bash
...
- start using dunst and making a configuration to have cool notifications. Could then make keys for changing brightness, volume, etc... Then I could also bind keys to calendar pop-ups, and generally make things more smooth.
